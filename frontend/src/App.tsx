...
function BooksLayout() {
  ...
  const listScrollRef = useScrollSound<HTMLDivElement>();
  const mainScrollRef = useScrollSound<HTMLDivElement>();
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      <Nav />
      <div className="flex-1 flex overflow-hidden">
        <div ref={listScrollRef} className="w-[30rem] border-r border-white/5 overflow-y-auto">
          <RowList selectedName={name ?? null} />
        </div>
        <main ref={mainScrollRef} className="flex-1 p-10 overflow-y-auto">
